...
public class Simulate
{
    ...
    //and edges every time step then calcule all the forces (gravity, wind and spring) and
    //fi corrections of the collisions, first plane collision and then
    //self collision (is posible to choose between self collisions
    //edge triangle collision or point triangle collision)
    public void Update(float dt, List<Triangles> triangles, List<Edges> edges)
    {
    ...
    void CheckPlaneCollitions(float dt, float frictionConstPlane, float dissipationConstPlane)
    {
        foreach(var p in particles)
        {
            //Here I do a projection only if the particle is on the wrong
            //side. Here the particle have to stay in the zone where the
            //normal of the plane is positive so if we do the dot product
            //between the normal plane and vector distance between the
            //plane and the point if dot product is negative the particle
            //is on the wrong side and I do a projection to put it in the
            //plane limit. In addition I also change the velocity to be able
    ...
    //Check plane collision (second plane)
